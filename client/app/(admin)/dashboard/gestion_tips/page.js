  ...
  return (
    <div className={`${navExpanded ? 'ml-56' : 'ml-12'} mt-8 transition-all duration-500`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold tracking-wide">Gestion des tips</h1>
        <button className="btn-style bg-green text-white border border-green hover:text-green hover:bg-white">&#10009; Ajouter un tips</button>
      </div>
              ...
              [...Array(55).keys()].slice(startIndex, startIndex + itemsPerPage).map((el, i) => {
                return (
                  <Suspense fallback={<Loading />} key={i}>
                    <tbody className="bg-white text-sm divide-y divide-gray text-darkgray">
                      <tr>
