...
export function DatePicker({ date, setDate }) {
  ...
  return (
    <div className="relative">
      {date && <span onClick={() =>setDate('')} className="absolute top-5 right-2 px-2 z-20 cursor-pointer">&#10005;</span>}
    
    <Popover>
      <PopoverTrigger asChild>
        <button
          variant={"outline"}
          className="flex items-center justify-between text-[#9CA3BC] w-72 md:w-96 rounded-md border border-gray py-2 px-4 my-3 outline-none"
        >
